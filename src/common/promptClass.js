...
import {
 ...
 export var popUp = {
    ...
    emptyChoose: '<p class="tips">未选择竞品，请先选择竞品<br><span style="font-size:14px;">现仅支持单项竞品加权</span></p><div class="cha-btns"><button class="btn hides">确定</button></div>',
     competingGoodsAnalysis: '<p class="head-title">竞品分析</p><div class="cross-tabs"><span class="cross-btn hasCross">跨类目商品</span><span class="cross-btn">监控商品</span></div><div class="analy-goods"><input type="text" class="anayEditor selcet" placeholder="请输入url或者商品id"><p class="good-tips"></p></div><div class="cha-btns"><button class="btn analyBtn">数据解析</button><button class="btn analyBtn hide-btn">流量解析</button><button class="btn analyBtn hide-btn">关键词解析</button></div><p class="bot-tips hide-btn">暂只支持同类目竞品解析</p>',
...
export function changeLoginStatus(type) {
    if (type == 'out') {
        ...
        $('.op-mc-search-analyze-container .ebase-Switch__root').html('<button id="search" class="serachBtn">一键分析</button>');
